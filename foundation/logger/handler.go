...
// logHandler provides a wrapper around the slog handler to capture which log level
// is being logged for event handling.
...
func newLogHandler(handler slog.Handler, events Events) *logHandler {
	return &logHandler{handler: handler, events: events}
}
